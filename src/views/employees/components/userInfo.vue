      ...
      <el-row class="inline-info">
        <el-col :span="12">
          <el-form-item label="员工头像">
            <!-- 放置上传图片 -->
            <image-upload ref="userInfoPhoto" />
          </el-form-item>
        </el-col>
      </el-row>
      <!-- 保存个人信息 -->
    ...
    <el-form label-width="220px">
      <div class="block">
        ...
        <el-form-item label="员工照片">
          <image-upload ref="formDataPhoto" />
        </el-form-item>
  ...
  methods: {
    ...
    saveUser() {
      try {
        ...
        const data = api.saveEmployees({ ...this.userInfo, staffPhoto: fileList && fileList.length ? fileList[0].url : ' ' })
    ...
    async savePersonal() {
      try {
        ...
        const data = await api.updataPersonalDetail({ ...this.formData, staffPhoto: fileList && fileList.length ? fileList[0].url : ' ' })
        console.log('基础信息data', data)
        this.$message.success('保存成功')
      } catch (err) {
        console.log('基础信息错误', err)
      }
    },
    async getUserBaseInfo() {
      try {
        const data = await api.getUserBaseInfo(this.userId)
    ...
    async getPersonalDetail() {
      try {
        const data = await api.getPersonalDetail(this.userId)
        this.formData = { ...data }
        this.$refs['formDataPhoto'].fileList = [{ url: this.formData.staffPhoto, upload: true }]
